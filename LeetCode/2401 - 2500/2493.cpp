...
// Solution 1: Using graph coloring and BFS (Longest Shortest Path)
...
public:
    int magnificentSets(int n, vector<vector<int>>& edges) {
        ...
        for (int node = 0; node < n; node++) {
            ...
            colors[node] = 0;
            // If the graph is not bipartite, return -1 (as we can't divide the nodes into groups with odd length cycles)
            if (!isBipartite(adjList, node, colors)) return -1;
        }

        // for each node, get the longest shortest path from that node
        // which gives us the maximum number of groups that can be formed with that node as the root
        vector<int> distances(n);
        for (int node = 0; node < n; node++) {
            distances[node] = getLongestShortestPath(adjList, node, n);
        }

        // for each component, get the maximum number of groups that can be formed
        // by traversing through all the nodes in that component and getting the longest shortest path (maximum number of groups)
        ...
        // return the maximum number of groups that can be formed by this graph
