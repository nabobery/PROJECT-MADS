...
// So for each T test there can be T+1  outcomes that is it can die in 1st or 2nd or Tth which result in T outcomes and the last case is it won't die! So total T+1 Outcomes are possible.
...
// (T+1)^p >= N
// Taking log on both sides we will get the required answer
// p >= log(N)/ log(T+1);
...
public:
    int poorPigs(int buckets, int minutesToDie, int minutesToTest) {
        ...
        return ceil(log2(buckets)/log2(attempts));
    }
};